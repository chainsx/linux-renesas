...
#define  USB_HOST_STR "host"
#define  USB_DEVICE_STR "peripheral"

/*  2023/05/18
	usb role is a fixed directory in use and is not friendly to use
*/
static int sys_usb_role_switch(int role_switch){
	struct file *file;
	char buffer[32];
	mm_segment_t fs;
	int ret;
    ...
    snprintf(path, sizeof(path), "/sys/devices/platform/soc/11c50200.usb-phy/role");
	...
	set_fs(KERNEL_DS);
#if 0
	ret = kernel_read(file, buffer, sizeof(buffer), 0);
	if (ret < 0) {
		return -1;
	}
	buffer[ret] = '\0';
	...
	{
	case USB_ROLE_HOST:
		snprintf(buffer, 5, USB_HOST_STR);
		break;
	case USB_ROLE_DEVICE:
		snprintf(buffer, sizeof(USB_DEVICE_STR), USB_DEVICE_STR);
		break;
	default:
		break;
	}

	ret = kernel_write(file, buffer, sizeof(USB_DEVICE_STR), 0);
...
static void ptn5150_check_state(struct ptn5150_info *info)
{
	...
	case PTN5150_DFP_ATTACHED:
		...
		sys_usb_role_switch(USB_ROLE_DEVICE);
		break;
	case PTN5150_UFP_ATTACHED:
		...
		sys_usb_role_switch(USB_ROLE_HOST);
		break;
	default:
		break;
	}
...
static int ptn5150_i2c_probe(struct i2c_client *i2c)
{
	...
	if (i2c->irq > 0) {
		...
		info->int_gpiod = devm_gpiod_get(&i2c->dev, "connect", GPIOD_IN);
